 ...
 * org.slf4j.LoggerFactory; import org.springframework.core.annotation.Order;
 * 
 * @Aspect public class LoggingAspect {
 * 
 * private static final Logger logger =
 * LoggerFactory.getLogger("com.ironManSocial.logger");
 * 
 * 
 * @Pointcut("execution(* *.*(..))") protected void loggingOperation() {}
 * 
 * @Before("loggingOperation()")
 * 
 * @Order(1) public void logJoinPoint(JoinPoint joinPoint){
 * logger.debug("Join point kind : " + joinPoint.getKind());
 * logger.debug("Signature declaring type : "+
 ...
 * 
 * @Order(2) public void logAfter(JoinPoint joinPoint, Object result){
 * logger.debug("Exiting from Method :"+joinPoint.getSignature().getName());
 * logger.debug("Return value :"+result); }
 * 
 * @AfterThrowing(pointcut="execution(* *.*(..))", throwing = "e")
 * 
 * @Order(3) public void logAfterThrowing(JoinPoint joinPoint, Throwable e){
